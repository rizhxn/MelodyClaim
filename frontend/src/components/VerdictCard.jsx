      ...
      <div className="relative z-10 p-8">
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-4">
            <div className={`p-4 rounded-full bg-black/50 border border-white/10 ${isStructural ? 'text-red-500' : isMinor ? 'text-yellow-500' : 'text-[#00ffcc]'}`}>
              {isNoMatch ? <CheckCircle2 className="w-8 h-8" /> : <AlertTriangle className="w-8 h-8" />}
            </div>
            <div>
              <h2 className="text-3xl font-bold text-white tracking-tight uppercase">
                {isStructural ? 'Structural Match' : isMinor ? 'Minor Overlap' : 'No Match Found'}
              </h2>
              <div className="flex items-center gap-2 mt-1 opacity-80">
                <span className="relative flex h-3 w-3">
                  {!isNoMatch && <span className={`animate-ping absolute inline-flex h-full w-full rounded-full opacity-75 ${isStructural ? 'bg-red-400' : 'bg-yellow-400'}`}></span>}
                  <span className={`relative inline-flex rounded-full h-3 w-3 ${isStructural ? 'bg-red-500' : isMinor ? 'bg-yellow-500' : 'bg-[#00ffcc]'}`}></span>
                </span>
                <span className="text-sm font-mono tracking-wider text-white/70">
                  {isNoMatch ? 'ORIGINAL COMPOSITION' : 'COPYRIGHT OVERLAP DETECTED'}
                </span>
              </div>
            </div>
          </div>
            ...
            <div className="lg:col-span-1 flex flex-col gap-3">
              <div className="bg-black/40 border border-white/5 rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-white/60">Match Length</span>
                </div>
                <span className="font-mono font-bold">{primaryMatch.matchLength} intervals</span>
              </div>

              <div className="bg-black/40 border border-white/5 rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Clock className="w-5 h-5 text-blue-400" />
                  <span className="text-sm text-white/60">Query Pos</span>
                </div>
                <span className="font-mono font-bold bg-white/10 px-2 py-1 rounded">{primaryMatch.queryStart}-{primaryMatch.queryEnd}</span>
              </div>

              <div className="bg-black/40 border border-white/5 rounded-xl p-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <Info className="w-5 h-5 text-teal-400" />
                  <span className="text-sm text-white/60">Ref Pos</span>
                </div>
                <span className="font-mono font-bold bg-white/10 px-2 py-1 rounded">{primaryMatch.referenceStart}-{primaryMatch.referenceEnd}</span>
              </div>
